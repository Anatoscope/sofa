...
static PyObject * Base_getPathName(PyObject * self, PyObject * /*args*/) {
    Base* obj = get_base(self);
    return PyString_FromString(obj->getPathName().c_str());
}

// the same as 'getPathName' with a extra prefix '@'
static PyObject * Base_getLinkPath(PyObject * self, PyObject * /*args*/) {
    Base* obj  = get_base(self);
    return PyString_FromString(("@"+obj->getPathName()).c_str());
}
...
SP_CLASS_METHOD(Base, getPathName)
SP_CLASS_METHOD(Base, getLinkPath)
...
static struct patch {
    patch() {
        SP_SOFAPYTYPEOBJECT(Base).tp_str = [](PyObject* self) {
            return Base_getLinkPath(self, nullptr);
        };
    }
} patcher;
